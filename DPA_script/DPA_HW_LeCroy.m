...
trc_matrix = zeros(traceLen,traceNum);

for n = 1:traceNum   
    
    traceFile = sprintf('C:\\Users\\afernandezrubio\\Desktop\\abraham\\python\\DPA\\Waveforms\\C1dpa_%05d.trc', (n-1) );
    ...
    % Get the file descriptor to extract the beginning of the samples
    data=fread(fid,50);
    WAVEDESC=findstr('WAVEDESC', char(data(1:50)'))-1;  

    % Get some parameters
    fseek(fid,WAVEDESC+ 32,'bof');
    COMM_TYPE = fread(fid,1,'int16');
    fseek(fid,WAVEDESC+ 36,'bof');
    WAVE_DESCRIPTOR = fread(fid,1,'int32');
    fseek(fid,WAVEDESC+ 40,'bof');
    USER_TEXT = fread(fid,1,'int32');
    fseek(fid,WAVEDESC+ 60,'bof');
    WAVE_ARRAY_1 = fread(fid,1,'int32');        
    fseek(fid, WAVEDESC + WAVE_DESCRIPTOR + USER_TEXT, 'bof');
    
    
    % Get the data from the trace files
    if COMM_TYPE == 0 % byte
        traces = fread(fid,WAVE_ARRAY_1, 'int8');
    else %word
        traces = fread(fid,WAVE_ARRAY_1, 'int16');
    end;
    fclose(fid);

    trc_matrix(:,n) = traces(1:traceLen) ;
